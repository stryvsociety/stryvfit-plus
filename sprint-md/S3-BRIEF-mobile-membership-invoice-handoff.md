...
- 2026-07-14: Deployment review found a lingering calendar-event environment binding and homepage nutrition card. Both are removed, and the active booking catalog no longer contains the deferred service object, so the public application bundle cannot carry meal-prep copy or configuration. Existing historical booking rows fall back to a neutral archived-service label rather than breaking list rendering.
- 2026-07-14: Invoice lifecycle review found that a zero-dollar tagged draft from an interrupted Stripe line-item call was excluded from the pending-invoice set. The retry now recovers that exact draft rather than creating a duplicate, and Stripe-mocked tests cover initial creation, zero-dollar draft recovery, card-only invoice settings, and the required free-session gate. Full verification passed: 83 tests / 282 assertions, typecheck, lint, build, audit, and diff check.
- 2026-07-14: Deployed recovery hardening as Cloudflare Worker version `2c7491c3-9ca7-4144-b868-bdc7c9a8ee6a` at 100%. The live anonymous invoice guard returned `401`, retired meal API returned `404`, and physical Chrome at 400px completed the public booking-to-Clerk sign-up handoff with the free-session redirect preserved and no `ERR_FAILED` state.
